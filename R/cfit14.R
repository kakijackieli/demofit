...
structure(
list(curve="Thatcher",x=x,m=m,w=w,A=A,B=B,C=C,D=D,fitted=fitted),
class="cFit14"
)
}
...
#' @export
deviance.cFit14 <- function(object,...) {
sum(object$w*(log(object$m)-log(object$fitted))^2)
}

#' @export
residuals.cFit14 <- function(object,...) {
