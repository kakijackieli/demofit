#' Poisson age-period-cohort model
#'
#' Fits and forecasts mortality rates using age-period-cohort model with Poisson assumption.
#'
...
#'
#' @details
#' The age-period-cohort (APC) model with Poisson assumption is specified as 
#' 
#' \eqn{ln(m_{x,t}) = \alpha_x + \kappa_t + \gamma_{t-x}} and \eqn{D_{x,t} ~ Poisson(E_{x,t} m_{x,t})}.
#'
...
#'
#' @return
#' An object of class PAPCS with associated S3 methods coef, forecast (which = 1 for smoothed (default); which = 2 for raw), plot, and residuals.
#'
#' @references
...
#' D <- round(E*M)
#' fit <- PAPCS(x=x,D=D,E=E,curve="makeham",h=30,jumpoff=2)
#' coef(fit)
...
#'
#' @export
PAPCS <- function(x,D,E,curve=c("gompertz","makeham","oppermann","thiele","wittsteinbumsted","perks","weibull","vandermaen","beard","heligmanpollard","rogersplanck","siler","martinelle","thatcher","gompertz2","makeham2","oppermann2","thiele2","wittsteinbumsted2","perks2","weibull2","vandermaen2","beard2","heligmanpollard2","rogersplanck2","siler2","martinelle2","thatcher2"),h=10,jumpoff=1) {
...
M <- D/E
nr <- nrow(M); nc <- ncol(M)
fit <- APCS(x,M,curve,h,jumpoff)
a <- coef(fit)$alpha
k <- coef(fit)$kappa
g <- coef(fit)$gamma
olde <- 1000000000; tol <- 1e-8
for (z in 1:200) {
amat <- matrix(a,nr,nc,byrow=TRUE); kmat <- matrix(k,nr,nc,byrow=FALSE); gmat <- g[row(M)-col(M)+nc]; Emat <- E*exp(amat+kmat+gmat)
a <- a+colSums(D-Emat)/colSums(Emat)
amat <- matrix(a,nr,nc,byrow=TRUE); kmat <- matrix(k,nr,nc,byrow=FALSE); gmat <- g[row(M)-col(M)+nc]; Emat <- E*exp(amat+kmat+gmat)
k <- k+rowSums(D-Emat)/rowSums(Emat); k <- k-mean(k)
amat <- matrix(a,nr,nc,byrow=TRUE); kmat <- matrix(k,nr,nc,byrow=FALSE); gmat <- g[row(M)-col(M)+nc]; Emat <- E*exp(amat+kmat+gmat)
...
dis <- sum(2*(D*log(D/Emat)-D+Emat))/(nr*nc-length(a)-length(k)-length(g)+2)
...
invisible(structure(
list(curve=curve,x=x,D=D,E=E,M=M,h=h,jumpoff=jumpoff,alpha=a,kappa=k,gamma=g,standardresiduals=res,dispersion=dis,forecast=Mf,smoothforecast=Mfs),
class="PAPCS"
))
...
#' @export
coef.PAPCS <- function(object,...) {
list(alpha=object$alpha,kappa=object$kappa,gamma=object$gamma)
}

#' @export
forecast.PAPCS <- function(object,which=1,...) {
...
#' @export
residuals.PAPCS <- function(object,...) {
