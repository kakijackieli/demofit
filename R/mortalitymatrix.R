#' Convert mortality data to matrix
#'
#' Converts mortality rates stored as a matrix, data frame, or demogdata object into required matrix format.
#'
#' @param data mortality data.
#' @param series mortality series index for demogdata objects.
#'
#' @return
#' A numeric matrix of mortality data.
#'
#' @examples
...
#' mortalitymatrix(Data)
#'
#' @export
mortalitymatrix <- function(data,series=1) {
if (is.matrix(data)) { return(data) }
if (is.data.frame(data)) { return(as.matrix(data)) }
if (inherits(data,"demogdata")) { return(data$rate[[series]]) } 
stop("data must be a matrix, data frame, or demogdata object")
}
