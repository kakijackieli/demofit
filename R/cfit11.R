...
suppressWarnings(result2 <- nlminb(c(0.0001,0.1,0.1,20),f2,lower=c(0,0,0,0),upper=c(Inf,Inf,Inf,Inf)))
...
suppressWarnings(resulta <- nlminb(param,f,lower=rep(0,9),upper=rep(Inf,9)))
h <- function(p) { sqrt(w)*(log(m)-log(p[1]+p[2]/exp(p[5]*x)+p[3]/exp(p[6]*(x-p[9])+1/exp(p[7]*(x-p[9])))+p[4]*exp(p[8]*x))) }
suppressWarnings(resultb <- nls.lm(param,h,lower=rep(0,9),upper=rep(Inf,9)))
...
structure(
list(curve="Rogers Planck",x=x,m=m,w=w,A0=A0,A1=A1,A2=A2,A3=A3,A=A,B=B,C=C,D=D,U=U,fitted=fitted,diagnostics=diagnostics),
class="cFit11"
)
}

#' @export
coef.cFit11 <- function(object,...) {
c(A0=object$A0,A1=object$A1,A2=object$A2,A3=object$A3,A=object$A,B=object$B,C=object$C,D=object$D,U=object$U)
}

#' @export
fitted.cFit11 <- .fittedFit

#' @export
predict.cFit11 <- function(object,newdata,...) {
...
#' @export
deviance.cFit11 <- .devianceFit

#' @export
residuals.cFit11 <- .residualsFit
