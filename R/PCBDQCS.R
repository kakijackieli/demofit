...
#' 
#' \eqn{ln(m_{x,t}) = \kappa_{1,t} + \kappa_{2,t} (x-\bar{x}) + \kappa_{3,t} ((x-\bar{x})^2-\sigma^2) + \gamma_{t-x}}
#' and \eqn{D_{x,t} ~ Poisson(E_{x,t} m_{x,t})}.
#'
...
#'
#' @export
PCBDQCS <- function(x,D,E,curve=c("gompertz","makeham","perks","weibull","beard","martinelle","thatcher","gompertz2","makeham2","perks2","weibull2","beard2","martinelle2","thatcher2"),h=10,jumpoff=1) {
...
M <- D/E
nr <- nrow(M); nc <- ncol(M)
fit <- CBDQCS(x,M,curve,h,jumpoff)
k1 <- coef(fit)$kappa1
k2 <- coef(fit)$kappa2
k3 <- coef(fit)$kappa3
g <- coef(fit)$gamma
...
for (z in 1:200) {
k1mat <- matrix(k1,nr,nc,byrow=FALSE); k2mat <- matrix(k2,nr,nc,byrow=FALSE); k3mat <- matrix(k3,nr,nc,byrow=FALSE); gmat <- g[row(M)-col(M)+nc]; xm <- matrix(x-mean(x),nr,nc,byrow=TRUE); xs <- matrix((x-mean(x))^2-s2,nr,nc,byrow=TRUE); Emat <- E*exp(k1mat+k2mat*xm+k3mat*xs+gmat)
k1 <- k1+rowSums(D-Emat)/rowSums(Emat)
k1mat <- matrix(k1,nr,nc,byrow=FALSE); k2mat <- matrix(k2,nr,nc,byrow=FALSE); k3mat <- matrix(k3,nr,nc,byrow=FALSE); gmat <- g[row(M)-col(M)+nc]; xm <- matrix(x-mean(x),nr,nc,byrow=TRUE); xs <- matrix((x-mean(x))^2-s2,nr,nc,byrow=TRUE); Emat <- E*exp(k1mat+k2mat*xm+k3mat*xs+gmat)
k2 <- k2+rowSums((D-Emat)*xm)/rowSums(Emat*xm^2)
k1mat <- matrix(k1,nr,nc,byrow=FALSE); k2mat <- matrix(k2,nr,nc,byrow=FALSE); k3mat <- matrix(k3,nr,nc,byrow=FALSE); gmat <- g[row(M)-col(M)+nc]; xm <- matrix(x-mean(x),nr,nc,byrow=TRUE); xs <- matrix((x-mean(x))^2-s2,nr,nc,byrow=TRUE); Emat <- E*exp(k1mat+k2mat*xm+k3mat*xs+gmat)
k3 <- k3+rowSums((D-Emat)*xs)/rowSums(Emat*xs^2)
k1mat <- matrix(k1,nr,nc,byrow=FALSE); k2mat <- matrix(k2,nr,nc,byrow=FALSE); k3mat <- matrix(k3,nr,nc,byrow=FALSE); gmat <- g[row(M)-col(M)+nc]; xm <- matrix(x-mean(x),nr,nc,byrow=TRUE); xs <- matrix((x-mean(x))^2-s2,nr,nc,byrow=TRUE); Emat <- E*exp(k1mat+k2mat*xm+k3mat*xs+gmat)
...
dis <- sum(2*(D*log(D/Emat)-D+Emat))/(nr*nc-length(k1)-length(k2)-length(k3)-length(g)+3)
...
invisible(structure(
list(curve=curve,x=x,D=D,E=E,M=M,h=h,jumpoff=jumpoff,kappa1=k1,kappa2=k2,kappa3=k3,gamma=g,standardresiduals=res,dispersion=dis,forecast=Mf,smoothforecast=Mfs),
class="PCBDQCS"
))
...
#' @export
residuals.PCBDQCS <- function(object,...) {
